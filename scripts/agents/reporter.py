...
def _format_robust_md(robust: list, stats: dict) -> str:
    """Render the cross-database consensus (robust conserved network) block."""
    if not robust:
        return ""
    min_support = stats.get("robust_min_support", 2)
    by_support = stats.get("by_support", {})
    tiers = ", ".join(f"{k} DB(s): {v}" for k, v in sorted(by_support.items()))
    lines = [
        "\n\n## Robust Conserved Network (cross-database consensus)",
        "",
        f"Directed interactions independently asserted by ≥{min_support} of the "
        f"{{SIGNOR, KEGG, Reactome}} sources — the conserved backbone that does not "
        f"depend on any single database.",
        "",
        f"- **Robust edges:** {stats.get('robust_edges', len(robust))} "
        f"over {stats.get('robust_proteins', '?')} proteins "
        f"(of {stats.get('unique_directed_pairs', '?')} total directed pairs)",
        f"- **Support distribution:** {tiers}",
        "",
        "| source | → | target | support | databases | effect(s) |",
        "|--------|---|--------|:-------:|-----------|-----------|",
    ]
    for r in robust[:25]:
        lines.append(f"| **{r['source']}** | → | **{r['target']}** | {r['support']} "
                     f"| {r['db_support']} | {r['effects']} |")
    if len(robust) > 25:
        lines.append(f"\n_…and {len(robust) - 25} more in `robust_edges.tsv`._")
    return "\n".join(lines) + "\n"
...
def reporter_node(state: PipelineState) -> dict:
    ...
    if edges:
        cols = ["source", "target", "effect", "mechanism", "db",
                "support", "db_support", "pathway_id"]
    ...
    # --- 3b. Robust conserved sub-network (directed pairs seen in >=2 DBs) ---
    robust = state.get("robust_edges", [])
    if robust:
        cols = ["source", "target", "support", "db_support", "effects", "mechanisms"]
        df_robust = pd.DataFrame(robust)
        df_robust = df_robust[[c for c in cols if c in df_robust.columns]]
        robust_file = out_dir / "robust_edges.tsv"
        df_robust.to_csv(robust_file, sep="\t", index=False)
        output_files.append(str(robust_file))
...
"""
    ...
    # Prepend an AI-model note so every report records which model/CLI produced it.
    # The model is the user-declared --model label (required); the CLI is detected.
    cli = active_cli_info()
    cli_label = f"{cli['name']} {cli['version']}".strip() or cli["name"]
    model_label = cli["model"] or "(unspecified)"
    model_note = f"{model_label} (user-declared)"
    header = (
        f"> **Model:** {model_note}  \n"
        f"> **LLM CLI:** {cli_label}  \n"
        f"> **Query:** {query}  \n"
    ...
    # Append the cross-database consensus (robust conserved network) block.
    report_text = report_text + _format_robust_md(
        state.get("robust_edges", []), state.get("network_stats", {}))
